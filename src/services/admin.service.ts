...
export class AdminService {
    ...
    static async createUserByAdmin(userData: any) {
        const { full_name, email, phone_number, password, planId, credit_balance } = userData;
    ...
    static async updateOnboardingRequest(requestId: string, status: 'APPROVED' | 'REJECTED', message?: string) {
    ...
    static async getFinancialStats() {
        ...
        const revenueByMonth = monthlyCredits.reduce((acc: any, credit) => {
            ...
            const month = credit.approved_at.toLocaleString('default', { month: 'short' });

...
import { PrismaClient, CreditRequestStatus } from '@prisma/client';
...
export class CreditController {
    static async createRequest(req: Request, res: Response) {
        try {
            ...
            if (!credits || !amount) {
                return res.status(400).json({ success: false, message: 'Invalid credits or amount' });
            }
    ...
    static async verifyPayment(req: Request, res: Response) {
        try {
            const { requestId, razorpay_payment_id, razorpay_order_id, razorpay_signature } = req.body;
            ...
            console.log(`[ADMIN NOTIFICATION SENT]: New Credit Recharge of ${creditRequest.credits} credits from ${creditRequest.user.email}. Requires manual approval.`);

            res.json({ success: true, message: 'Payment verified successfully. Awaiting admin approval.' });
        } catch (error: any) {
    ...
    static async approveRequest(req: Request, res: Response) {
        try {
            ...
            if (!creditRequest || creditRequest.status !== 'PAID') {
                return res.status(400).json({ success: false, message: 'Invalid or unpaid request' });
            }
            ...
            res.json({ success: true, message: 'Credit request approved and balance updated.' });
        } catch (error: any) {
